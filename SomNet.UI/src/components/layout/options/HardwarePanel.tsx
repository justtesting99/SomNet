...
export function HardwarePanel({ active }: HardwarePanelProps) {
  ...
  useEffect(() => {
    if (!active || systemStatus.lastChecked === null) {
      return;
    }
  ...
  }, [active, systemStatus.lastChecked, loadAll]);
      ...
      {activeTab === 'online' ? (
        <div className="space-y-4">
          <p className="text-sm text-slate-400">
            ...
            <span className="text-slate-300">without a pairing token</span> — e.g. new units, after
            factory reset, or after <span className="text-slate-300">Revoke</span> on All Subs.
            Hardware already paired to a Sub stays on{' '}
            ...
            <>
              ...
              {pairedOnlineRows.length > 0 ? (
                ...
                <p className="text-sm text-slate-500">
                  No hardware is connected. Use Enter device ID if the device has not joined the hub
                  yet.
                </p>
              )}
            </>
